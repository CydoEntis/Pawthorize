...
/// </summary>
internal static class TokenDeliveryHelper
{
    /// <summary>
        ...
        string? csrfToken = null)
    {
        ...
        try
        {
            var result = strategy switch
            {
                ...
                TokenDeliveryStrategy.HttpOnlyCookies => DeliverInCookies(authResult, httpContext, options, csrfService,
                    logger, csrfToken),
    ...
    private static IResult DeliverInCookies(
        ...
        string? csrfToken = null)
    {
        logger?.LogDebug("Delivering access and refresh tokens in HttpOnly cookies, RememberMe: {RememberMe}",
            authResult.IsRememberedSession);
        ...
        SetRefreshTokenCookie(httpContext, authResult.RefreshToken!, authResult.RefreshTokenExpiresAt, useSessionCookie,
            logger);
    ...
    {
        logger?.LogDebug(
            "Delivering tokens in hybrid mode (access token in body, refresh token in cookie), RememberMe: {RememberMe}",
            authResult.IsRememberedSession);
    ...
    /// </summary>
    private static void SetRefreshTokenCookie(HttpContext context, string value, DateTime expires,
        bool useSessionCookie, ILogger? logger)
    {
        if (useSessionCookie)
        {
        ...
        string? preGeneratedToken = null)
    {
        ...
        logger?.LogDebug(
            "CSRF token set in cookie '{CookieName}' and header '{HeaderName}', SessionCookie: {UseSessionCookie}",
